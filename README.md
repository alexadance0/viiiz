...
Поток рендера: editor config → `ChartDocument` → `ChartPlugin.compile` → `ChartScene` → layout/renderer. Обычные bar/stacked/horizontal-bar, basic line/spline/step-line, area/stacked-area, indexed/seasonal trends, Slope, smoothing, interval, Scatter/Bubble и девять lane-based Distribution видов используют native semantic scene. Box/Violin/Raincloud/Ridgeline компилируют renderer-neutral summary/density layers; Histogram и KDE пока явно остаются на legacy frequency path. Новый график добавляется семантическим plugin compiler и регистрацией в `chartRegistry`; UI выбора строится из реестра и semantic capabilities.
