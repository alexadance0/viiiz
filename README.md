...
Поток рендера: editor config → `ChartDocument` → `ChartPlugin.compile` → `ChartScene` → layout/renderer. Обычные bar/stacked/horizontal-bar, basic line/spline/step-line, area/stacked-area, indexed/seasonal trends и Slope используют native semantic scene; Slope имеет отдельный comparison-layout внутри общего document frame, включая единственное владение коллизиями endpoint-подписей и геометрию подписей изменения. Направление и формат изменения задаёт общий renderer-neutral helper, используемый также Dumbbell. Остальные специализированные семейства явно остаются на legacy compatibility path. Новый график добавляется семантическим plugin compiler и регистрацией в `chartRegistry`; UI выбора строится из реестра и semantic capabilities.
