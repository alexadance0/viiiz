...
Поток рендера: editor config → `ChartDocument` → `ChartPlugin.compile` → `ChartScene` → layout/renderer. Обычные bar/stacked/horizontal-bar, basic line/spline/step-line, area/stacked-area, indexed/seasonal trends, Slope, smoothing, interval, Scatter/Bubble и все одиннадцать Distribution видов используют native semantic scene. Box/Violin/Raincloud/Ridgeline компилируют renderer-neutral summary/density layers, а Histogram/KDE — frequency layers с непрерывными осями значения и частоты/плотности. Новый график добавляется семантическим plugin compiler и регистрацией в `chartRegistry`; UI выбора строится из реестра и semantic capabilities.
