...
Поток рендера: editor config → `ChartDocument` → `ChartPlugin.compile` → `ChartScene` → layout/renderer. Обычные bar/stacked/horizontal-bar, basic line/spline/step-line, area/stacked-area, indexed/seasonal trends, Slope, smoothing, interval, Scatter и Bubble используют native semantic scene. Scatter/Bubble имеют отдельный `plot.kind = 'xy'` с двумя непрерывными осями, row-stable точками, size encoding, категориальной легендой и renderer-neutral analytical layers. Остальные специализированные семейства явно остаются на legacy compatibility path. Новый график добавляется семантическим plugin compiler и регистрацией в `chartRegistry`; UI выбора строится из реестра и semantic capabilities.
