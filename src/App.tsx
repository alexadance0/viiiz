...
function App() {
  ...
  useEffect(() => {
    const updateCategoryLabel = (event: Event) => {
      const detail = (event as CustomEvent<{ axis: 'x' | 'y'; category: string; text: string }>).detail
      if (!detail) return
      setConfig((current) => ({ ...current, categoryLabelOverrides: { ...current.categoryLabelOverrides, [detail.axis]: { ...current.categoryLabelOverrides?.[detail.axis], [detail.category]: detail.text } } }))
    }
    window.addEventListener('viiiz:category-label-change', updateCategoryLabel)
    return () => window.removeEventListener('viiiz:category-label-change', updateCategoryLabel)
  }, [])

  useEffect(() => {
    const reorderTreemap = (event: Event) => {
      const detail = (event as CustomEvent<{ source: ChartElementSelection; target: ChartElementSelection; placement: 'before' | 'after' }>).detail
      if (detail) moveTreemapElement(detail.source, detail.target, detail.placement)
    }
    window.addEventListener('viiiz:treemap-move', reorderTreemap)
    return () => window.removeEventListener('viiiz:treemap-move', reorderTreemap)
  })
