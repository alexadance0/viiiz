...
export interface ResolvedDistributionHistogramBin { mark: DistributionHistogramBinScene; rect: Rect; label?: { x: number; y: number; align: 'center' | 'left'; verticalAlign: 'bottom' | 'middle' } }
export interface ResolvedDistributionKdeShape { layerId: LayerId; groupId: DistributionGroupId; samples: Array<{ value: number; density: number }>; points: number[][]; polygon: number[][]; stroke: { color: string; width: number }; fill: { color: string; opacity: number }; tooltip: { group: string } }
export interface ResolvedDistributionGeometry { marks: ResolvedDistributionMarkPlacement[]; summaries: ResolvedDistributionSummaryPlacement[]; boxShapes: ResolvedDistributionBoxShape[]; densityShapes: ResolvedDistributionDensityShape[]; densitySummaries: ResolvedDistributionDensitySummary[]; frequencyBins: ResolvedDistributionHistogramBin[]; frequencyCurves: ResolvedDistributionKdeShape[]; frequencySummaries: ResolvedDistributionSummaryPlacement[]; laneGrid: ResolvedDistributionLaneGridLine[]; laneBand: number }
...
export function resolveNativeDistributionScene(scene: NativeDistributionChartScene): ResolvedDistributionScene {
  ...
  if (scene.plot.variant === 'histogram' || scene.plot.variant === 'kde') {
    const frequency = scene.plot.frequencyDomain!, amountPixel = (amount: number) => project(amount, frequency.min, frequency.max, horizontal ? frame.plot.y : frame.plot.x, horizontal ? frame.plot.height : frame.plot.width, horizontal)
    const histogramLayer = scene.plot.layers.find((layer) => layer.kind === 'histogram'), kdeLayer = scene.plot.layers.find((layer) => layer.kind === 'kde'), summaryLayer = scene.plot.layers.find((layer) => layer.kind === 'frequency-summaries')
    const frequencyBins = histogramLayer?.groups.flatMap((entry) => entry.bins.map((mark): ResolvedDistributionHistogramBin => { const zero = amountPixel(0), top = amountPixel(mark.amount), first = valuePixel(mark.start), second = valuePixel(mark.end); const rect = horizontal ? { x: Math.min(first, second), y: Math.min(zero, top), width: Math.max(1, Math.abs(second - first) - 1), height: Math.abs(zero - top) } : { x: Math.min(zero, top), y: Math.min(first, second), width: Math.abs(zero - top), height: Math.max(1, Math.abs(second - first) - 1) }; return { mark, rect, ...(mark.label ? { label: horizontal ? { x: rect.x + rect.width / 2, y: rect.y - 5, align: 'center' as const, verticalAlign: 'bottom' as const } : { x: rect.x + rect.width + 5, y: rect.y + rect.height / 2, align: 'left' as const, verticalAlign: 'middle' as const } } : {}) } })) ?? []
    const frequencyCurves = kdeLayer?.groups.map((entry): ResolvedDistributionKdeShape => { const points = entry.points.map((point) => horizontal ? [valuePixel(point.value), amountPixel(point.density)] : [amountPixel(point.density), valuePixel(point.value)]), zero = amountPixel(0), polygon = horizontal ? [[points[0][0], zero], ...points, [points.at(-1)![0], zero]] : [[zero, points[0][1]], ...points, [zero, points.at(-1)![1]]]; return { layerId: entry.id, groupId: entry.groupId, samples: entry.points, points, polygon, stroke: entry.stroke, fill: entry.fill, tooltip: entry.tooltip } }) ?? []
    const frequencySummaries = summaryLayer?.marks.map((mark): ResolvedDistributionSummaryPlacement => { const primary = valuePixel(mark.value), start = amountPixel(0), end = amountPixel(mark.amount * mark.lengthRatio); return horizontal ? { id: mark.id, groupId: mark.groupId, x1: primary, y1: start, x2: primary, y2: end, color: mark.color, width: mark.width, cap: 'butt' } : { id: mark.id, groupId: mark.groupId, x1: start, y1: primary, x2: end, y2: primary, color: mark.color, width: mark.width, cap: 'butt' } }) ?? []
    ...
    return { ...scene, plot: { ...scene.plot, valueAxis, laneAxis, frequencyAxis: laneAxis }, geometry, resolvedReservations: frame.resolvedReservations, distributionGeometry: { marks: [], summaries: [], boxShapes: [], densityShapes: [], densitySummaries: [], frequencyBins, frequencyCurves, frequencySummaries, laneGrid: [], laneBand: 0 } }
  }
  ...
  return { ...scene, plot: { ...scene.plot, valueAxis, laneAxis }, geometry, resolvedReservations: frame.resolvedReservations, distributionGeometry: { marks, summaries, boxShapes, densityShapes, densitySummaries, frequencyBins: [], frequencyCurves: [], frequencySummaries: [], laneGrid, laneBand } }
}
