...
function measuredAxis(scene: NativeChartScene, source: AxisSpec, estimatedPlot: Rect): AxisSpec {
  ...
  const formatter = scene.plot.orientation === 'horizontal' ? formatXAxisNumber : formatYAxisNumber
...
export function resolveNativeBarScene(scene: NativeChartScene): ResolvedScene & NativeChartScene {
  const config = scene.compatibilityConfig
  ...
  if (scene.plot.orientation === 'horizontal' && config.showDirectLabels && valueAxis.placement.kind === 'side' && valueAxis.placement.side === 'top') {
  ...
  if (scene.plot.orientation === 'vertical' && valueAxis.labels.visible && valueAxis.placement.kind === 'side') {
    reservations.push({ id: 'axis:value-label-safety', side: valueAxis.placement.side, size: 2, gap: 0, mode: 'outside', priority: 51 })
  }
  if (scene.plot.orientation === 'vertical' && categoryAxis.labels.visible) {
    const finalLabel = scene.plot.categories.findLast((category) => Boolean(category.label))?.label ?? ''
    const longestLine = Math.max(0, ...finalLabel.split('\n').map((line) => line.length))
    const edge = Math.min(110, Math.ceil(Math.max(measureTextWidth(finalLabel, categoryAxis.labels.style.size, categoryAxis.labels.style.fontFamily, categoryAxis.labels.style.weight) / 2, longestLine * categoryAxis.labels.style.size * .3)) + 14)
    if (edge) reservations.push({ id: 'axis:category-edge', side: 'right', size: edge, gap: 0, mode: 'outside', priority: 55 })
  }
  if (scene.plot.orientation === 'vertical' && valueAxis.labels.visible) {
  ...
  if (scene.plot.orientation === 'horizontal' && valueAxis.labels.visible) {
    const formatter = scene.plot.stacking === 'normalized' ? formatYAxisNumber : formatXAxisNumber
  ...
  if (config.showValues && config.barValueLabelAbsorption && !(config.valueLabelPosition ?? '').startsWith('inside-')) {
  ...
  scene.plot.categories.forEach((category, index) => {
    if (scene.plot.orientation === 'vertical') {
