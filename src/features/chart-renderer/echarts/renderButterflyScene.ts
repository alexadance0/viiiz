...
export function renderButterflyScene(scene: ResolvedButterflyScene): Record<string, unknown> {
  ...
  const series = scene.plot.series.map((item) => {
    ...
    return label ? { type: 'group', silent: false, info, children: [rect, { type: 'text', silent: false, info: { ...info, selectionTarget: 'value-label' }, style: { ...label, text: mark.label.text, ...text(mark.label.style), fill: label.inside && mark.label.autoContrast ? contrast(mark.style.color) : mark.label.style.color } }] } : rect
    } }
  })
  ...
  const markHits = scene.plot.series.flatMap((item) => item.marks.flatMap((mark) => scene.butterflyGeometry.marks[mark.id] ? [{ rect: scene.butterflyGeometry.marks[mark.id], info: { elementId: mark.id, elementKey: mark.legacyKey, sourceSeriesName: item.name, displayCategory: mark.displayCategory, displayValue: mark.displayValue, displayColor: mark.style.color, selectionMode: 'series-first' as const } }] : []))
  ...
  if (!centered) return { ...base, nativeSelectionHits, nativeCategoryLayouts, xAxis, yAxis, series, graphic: [...((base.graphic as unknown[]) ?? []), ...centerLabels] }
  ...
  return { ...base, nativeSelectionHits, nativeCategoryLayouts, grid: grids, xAxis: [leftAxis, rightAxis], yAxis: [leftCategory, rightCategory], series, graphic: [...((base.graphic as unknown[]) ?? []), ...centerLabels] }
}
