...
import type { SlopeEndpointLabelScene } from '../../../entities/chart/model/ChartScene'
...
const endpointText = (item: SlopeEndpointLabelScene) => [item.valueText, item.seriesText].filter(Boolean).join(' ')
...
export function renderNativeSlopeScene(scene: ResolvedSlopeScene): Record<string, unknown> {
  ...
  const labels = new Map(scene.plot.endpointLabels.items.map((item) => [item.pointId, item]))
  ...
  const series = scene.plot.series.map((item) => ({
    ...
    data: item.points.map((point) => {
      ...
      return {
        ...
        label: label ? { show: true, formatter: endpointText(label), position: label.side, distance: scene.plot.endpointLabels.distance, offset: [0, scene.slopeGeometry.endpointLabelOffsets[label.id] ?? 0], ...textStyle(label.style), color: label.color } : { show: false },
      }
    }),
  }))
  ...
  return {
    ...
    tooltip: { trigger: 'axis', formatter: (input: unknown) => { const items = (Array.isArray(input) ? input : [input]) as Array<{ seriesName?: string; data?: { displayCategory?: string; displayValue?: string } }>; const visible = items.filter((entry) => entry.seriesName && !entry.seriesName.startsWith('__')); return [`<b>${escapeHtml(visible[0]?.data?.displayCategory ?? '')}</b>`, ...visible.map((entry) => `${escapeHtml(entry.seriesName)}: <b>${escapeHtml(entry.data?.displayValue ?? '')}</b>`)].join('<br/>') } },
    legend: { show: false },
    ...
    series: [...series, ...hits], graphic: [...verticalTitle, ...guideGraphics(scene), ...footer],
  }
}
