import type { ChartScene, NativeButterflyChartScene, NativeDistributionChartScene, NativeSlopeChartScene, NativeWaterfallChartScene, NativeXYChartScene, ResolvedScene } from '../../../entities/chart/model/ChartScene'
...
export function renderScene(scene: ChartScene | ResolvedScene): Record<string, unknown> {
  ...
  const resolved = 'geometry' in scene ? scene : scene.plot.kind === 'slope' ? resolveNativeSlopeScene(scene as NativeSlopeChartScene) : scene.plot.kind === 'xy' ? resolveNativeXYScene(scene as NativeXYChartScene) : scene.plot.kind === 'distribution' ? resolveNativeDistributionScene(scene as NativeDistributionChartScene) : scene.plot.kind === 'waterfall' ? resolveNativeWaterfallScene(scene as NativeWaterfallChartScene) : scene.plot.kind === 'butterfly' ? resolveNativeButterflyScene(scene as NativeButterflyChartScene) : resolveNativeCartesianScene(scene)
