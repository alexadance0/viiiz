...
export function DataTransformDialog({ table, types, onApply, onClose }: Props) {
  ...
  return <div className="dialog-backdrop" onMouseDown={(event) => event.target === event.currentTarget && onClose()}><section className="date-dialog fred-dialog" role="dialog" aria-modal="true">
    ...
    <Tabs className="settings-tabs dialog-tabs series-tabs fred-tabs" selectedKey={tab} onSelectionChange={(key) => setTab(String(key) as Tab)}>
      <Tabs.ListContainer><Tabs.List aria-label="Настройка временного ряда">{([['transform', 'Единицы и частота'], ['fill', 'Заполнить пропуски'], ['smooth', 'Сглаживание']] as const).map(([key, label]) => <Tabs.Tab id={key} key={key}><Tabs.Indicator/>{label}</Tabs.Tab>)}</Tabs.List></Tabs.ListContainer>
      <Tabs.Panel id={tab}><div className="dialog-body fred-body">
    ...
    </Tabs><footer><button className="button" onClick={onClose}>Отмена</button><button className="button primary" disabled={disabled} onClick={apply}>{tab === 'fill' ? 'Заполнить периоды' : tab === 'smooth' ? 'Создать столбец' : 'Применить'}</button></footer>
  </section></div>
}
