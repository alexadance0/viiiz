...
interface Props { fileName: string; sheets: Array<{ name: string; table: DataTable }>; onSelect(table: DataTable): void; onClose(): void }

export function ExcelSheetDialog({ fileName, sheets, onSelect, onClose }: Props) {
  return <div className="dialog-backdrop" onMouseDown={(event) => event.target === event.currentTarget && onClose()}><section className="date-dialog sheet-dialog" role="dialog" aria-modal="true">
    <header><div><span className="eyebrow">Книга Excel</span><h2>Выберите лист</h2><p>{fileName} · найдено листов: {sheets.length}</p></div><button onClick={onClose}>×</button></header>
