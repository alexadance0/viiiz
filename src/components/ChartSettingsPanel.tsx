...
function SeriesDirectLabelSettings({ name, config, onChange }: { name: string; config: ChartConfig; onChange(values: Partial<ChartConfig['seriesStyles'][string]>): void }) {
  const style = config.seriesStyles[name]
  const visible = style?.showDirectLabel !== false
...
export function ChartSettingsPanel({ config, plugin, seriesNames, valueLabels, xKind, frequency, onChange }: Props) {
  ...
  const resetSection = (section: 'grid' | 'headings' | 'axes' | 'legend' | 'values' | 'credits' | 'bars') => {
    ...
    if (section === 'axes') patch({ xAxisPosition: 'bottom', yAxisPosition: 'left', showXAxisTitle: false, showYAxisTitle: false, showXAxisLabels: true, showYAxisLabels: true, showXAxisLine: true, showYAxisLine: false, showXTicks: true, showYTicks: false, tickLength: 6, axisLineColor: '#55515e', axisLineWidth: 1, axisLineType: 'solid', xAxisTitleGap: 14, yAxisTitleGap: 14, xAxisLabelGap: 8, yAxisLabelGap: 8, yAxisScaleType: 'linear', showZeroLine: false, zeroLineColor: '#8a8791', zeroLineWidth: 1, zeroLineType: 'solid', xAxisTitleText: defaultText(20, 600), yAxisTitleText: defaultText(20, 600), xAxisLabelText: defaultText(18, 400, '#555555'), yAxisLabelText: defaultText(18, 400, '#555555') })
    if (section === 'legend') patch({ showLegend: false, showDirectLabels: false, legendPosition: 'top', legendText: defaultText(19), directLabelText: defaultText(19, 600), directLabelGap: 16, showDirectLabelLines: false, directLabelLineWidth: 1, directLabelLineType: 'solid', seriesStyles: Object.fromEntries(Object.entries(config.seriesStyles).map(([name, style]) => { const { legendLabel: _label, legendNote: _note, showDirectLabel: _show, directLabelText: _text, showLegendLine: _line, ...rest } = style; return [name, rest] })) })
    ...
    <details className="settings-group legend-settings"><summary>Легенда</summary><div>
      {!slope && config.kind !== 'heatmap' && config.kind !== 'treemap' && <><div className="legend-mode-label">Способ обозначения рядов <Hint text="Прямые подписи называют линии или ряды прямо на графике и часто заменяют отдельную легенду."/></div><Tabs className="settings-tabs legend-mode" selectedKey={legendMode} onSelectionChange={(key) => setLegendMode(String(key) as 'none' | 'standard' | 'direct')}>
        ...
        {plugin.settings.features.directLabels && <Tabs.Panel id="direct"><div className="legend-options direct-legend-options">
          ...
          <div className="series-label-list"><span>Подписи по рядам</span>{seriesNames.map((name) => <SeriesDirectLabelSettings name={name} config={config} onChange={(values) => updateSeriesLegend(name, values)} key={name}/>)}</div>
        </div></Tabs.Panel>}
      </Tabs></>}
