...
export function SettingsQuickNav({ features, showSeries }: Props) {
  const items = [['Холст', 'Холст'], ['Заголовок и подзаголовок', 'Заголовок и подзаголовок'], ['Общий стиль текста', 'Общий стиль текста'], ['Палитра', 'Палитра']]
  ...
  items.push(['Оси', 'Оси'], ['Шкалы и подписи осей', 'Шкалы и подписи осей'], ['Сетка', 'Сетка'], ['Формат чисел', 'Формат чисел'], ['Легенда и значения', 'Легенда и значения'], ['Аннотации и акценты', '.annotation-add-wrap'], ['Комментарий и источник', 'Комментарий и источник'])
