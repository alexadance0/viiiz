...
export function LineVariantSettings({ config, numericColumns = config.yFields, table, onChange }: Props) {
  ...
  if (config.kind === 'slope') return <details className="settings-group line-variant-settings"><summary>Наклонный график</summary><div>
    ...
    <SettingsCheckbox isSelected={config.slopeShowChange ?? false} onChange={(slopeShowChange) => patch({ slopeShowChange })}>Показывать изменение</SettingsCheckbox>
    {(config.slopeShowChange ?? false) && <>
      <label>Формат<select value={config.slopeChangeFormat ?? 'absolute'} onChange={(event) => patch({ slopeChangeFormat: event.target.value as ChartConfig['slopeChangeFormat'] })}><option value="absolute">Абсолютное значение</option><option value="percent">Процентное изменение</option></select></label>
      {(config.slopeChangeFormat ?? 'absolute') === 'percent' && <label>Знаков после запятой<NumberInput min="0" max="6" value={config.slopeChangePercentDecimals ?? 0} onValueChange={(slopeChangePercentDecimals) => patch({ slopeChangePercentDecimals: Math.round(slopeChangePercentDecimals) })}/></label>}
      <label>Расположение<select value={config.slopeChangePosition ?? 'middle'} onChange={(event) => patch({ slopeChangePosition: event.target.value as ChartConfig['slopeChangePosition'] })}><option value="start">Ближе к началу</option><option value="middle">Посередине</option><option value="end">Ближе к концу</option></select></label>
    </>}
    <SettingsCheckbox isSelected={config.slopeColorByChange ?? false} onChange={(slopeColorByChange) => patch({ slopeColorByChange })}>Цвет по направлению изменения</SettingsCheckbox>
